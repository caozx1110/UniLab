...
3. 在 env 的 `__init__` 冷路径、backend 解析 XML 之前调用目录 resolver。
   现有 API 一次只解析一个目录，因此 T800 需要调用两次：
...
- 机器人二进制资产使用同一目录 resolver（`resolve_robot_asset_dir`）。X2 集成点为
  `src/unilab/envs/motion_tracking/x2/flip_tracking.py` 中的
  `X2WallFlipTrackingEnv.__init__`；T800 集成点为
  `src/unilab/envs/locomotion/t800/joystick.py` 中的 `T800WalkEnv.__init__`。
  两者均通过 `unilab-pull-assets` CLI 暴露。
