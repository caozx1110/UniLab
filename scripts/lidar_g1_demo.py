"""Interactive G1 LiDAR demo on mujoco-uni 3.8.0.post1's batched raycaster.

This reproduces MuJoCo-LiDAR's ``examples/unitree_g1.py --backend warp``
(interactive MuJoCo window + Unitree G1 stepping via an ONNX policy + a mid360
LiDAR point cloud colored by height), but the LiDAR ranges are produced by
...
MuJoCo-LiDAR project and contains no absolute paths. This is the G1 counterpart
of ``scripts/lidar_go2_demo.py``.
...
Run (call the venv python directly for a clean exit code):
    ...
    ./.venv/bin/python scripts/lidar_g1_demo.py
    ./.venv/bin/python scripts/lidar_g1_demo.py --lidar mid360

`uv run --no-sync python scripts/lidar_g1_demo.py` also works, but in this env
...
class BatchRayLidar:
    ...
    def __init__(
        ...
        site_name: str = "lidar",
        bodyexclude_name: str = "torso_link",
...
# ---------------------------------------------------------------------------
# ONNX walking controller (verbatim port of MuJoCo-LiDAR examples/unitree_g1.py).
# ---------------------------------------------------------------------------
class OnnxController:
    ...
    def get_obs(self, mj_model: mujoco.MjModel, mj_data: mujoco.MjData) -> np.ndarray:
        linvel = mj_data.sensor("local_linvel_pelvis").data
        gyro = mj_data.sensor("gyro_pelvis").data
        imu_xmat = mj_data.site_xmat[mj_model.site("imu_in_pelvis").id].reshape(3, 3)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="G1 LiDAR demo using mujoco-uni post1 BatchEnvPool.multi_ray"
    )
    ...
    scene_path = _ASSETS / "robots" / "g1" / "scene_g1.xml"
    policy_path = _ASSETS / "checkpoints" / "lidar_demo" / "g1_policy.onnx"
