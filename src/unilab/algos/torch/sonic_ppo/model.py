...
class UniversalToken(nn.Module):
    ...
    def route(self, observations: torch.Tensor) -> tuple[torch.Tensor, dict[str, object]]:
        ...
        for encoder_name in self.encoder_names:
            ...
            if flat_tokens.dtype != tokens.dtype:
                flat_tokens = flat_tokens.to(dtype=tokens.dtype)
...
class SonicActorCritic(nn.Module):
    ...
    def value(self, critic_obs: torch.Tensor) -> torch.Tensor:
        """Evaluate the value head from critic observations only.

        Manager-based timeout bootstrap must not require actor/tokenizer input
        groups.  This explicit critic-only path also keeps that operation
        independent of actor routing and token quantization.
        """

        if critic_obs.ndim < 1 or critic_obs.shape[-1] != self.critic_obs_dim:
            raise ValueError(
                f"critic expects {self.critic_obs_dim} features, got {tuple(critic_obs.shape)}"
            )
        critic_features = self.critic_rms(critic_obs) if self.critic_rms is not None else critic_obs
        return self.critic(critic_features).squeeze(-1)
